...
impl<const MAX_PACKET_SIZE: usize> NetworkManager<MAX_PACKET_SIZE> {
    ...
    pub fn payload_to_send(
        ...
        let new_pkt = self.new_packet(payload, destination, pkt_type)?;
        to_send
            .push(new_pkt)
            .expect("The above check should mean this never errors out");
        Ok(to_send)
    }

    pub fn add_packet(&mut self, packet: MHPacket) -> Result<(), NetworkManagerError> {
        let curr_time = Instant::now(); // + Instant::from_secs(self.timeout as u64);
    ...
    pub fn receive_packet(
        &mut self,
        pkt: MHPacket,
    ) -> Result<Option<(MHPacket, PayloadType)>, NetworkManagerError> {
    ...
    pub fn handle_packets(
        &mut self,
        pkts: Vec<MHPacket, MAX_AMOUNT_PACKETS>,
    ) -> Result<
        (
            Vec<MHPacket, MAX_AMOUNT_PACKETS>,
            Vec<MHPacket, MAX_AMOUNT_PACKETS>,
        ),
        NetworkManagerError,
    > {
        let mut to_send: Vec<MHPacket, MAX_AMOUNT_PACKETS> = Vec::new();
        let mut commands: Vec<MHPacket, MAX_AMOUNT_PACKETS> = Vec::new();
        for pkt in pkts {
