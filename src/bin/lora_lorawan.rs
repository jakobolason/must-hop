...
async fn main(_spawner: Spawner) {
    ...
    let config = sx126x::Config {
        ...
        tcxo_ctrl: None, // Some(TcxoCtrlVoltage::Ctrl1V7),
        use_dcdc: true,
        rx_boost: false,
    };
    ...
    loop {
        ...
        let resp = match device
            .join(&JoinMode::OTAA {
                deveui: DevEui::from([0xAC, 0x1F, 0x09, 0xFF, 0xFE, 0x1A, 0xCC, 0x0A]),
                appeui: AppEui::from([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
                appkey: AppKey::from([
                    0x4C, 0xD4, 0xD2, 0x01, 0x35, 0x63, 0x43, 0xCC, 0x10, 0x04, 0xDF, 0x14, 0xBC, 0x24, 0x62, 0xD0,
                ]),
            })
            .await
        {
